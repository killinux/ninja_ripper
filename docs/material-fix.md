...
reconstruct()                 # 投影无关的 clip-space 重建（拼装身体 + 去重复 pass + 跨pass恢复眼球）
...
眼球几何恢复后，自动材质给它配的是**眼白(sclera)** 贴图、且落在退化的 uv 层上 →
"眼睛全白"。排查眼球绑定的贴图：

| 贴图 | 是什么 | 贴到眼球上的效果 |
|---|---|---|
| `30D730` | **完整眼球图**（眼白 + 位置正确的虹膜） | 配 `uv_0` 时眼睛自然就位（当前用这张） |
| `A9F630` | **纯虹膜图**（放射纹理 + 瞳孔） | 铺满整球→像一颗突出的纯蓝球，大小/位置都不对 |
| `30DD30` | 黑底 + 橙色圆点（瞳孔高光/自发光） | 暗琥珀色 |
| `A9FC30` / `A9ED70` | 法线 / 阴影 | — |

两个启发式都对眼球失灵：① 内容分类挑了浅色的眼白(30D730)而非虹膜；② UV 连续性挑了
**退化层 uv_7**（整球塌缩到一点 → 全白）。修法：
- `_coherent_uv` 增加**退化层防护**（UV 包围盒面积过小的层跳过），通用。
- 眼球这种"内容/UV 都猜不准"的特例，用 **`ALBEDO_OVERRIDES`**（按 PreVS 顶点数指定
  `贴图 + uv 层`）显式钉死：本模型 `458 -> (30D730, uv_0)`。

**仍不完美（已知，暂搁置）**：单张 base-color 贴图无法还原游戏"眼白 + 正面虹膜 +
瞳孔 + 高光"的分层眼睛 shader；当前虹膜颜色偏淡、不够"蓝"，眼球也略显整体一色。
要更准需要把眼白/虹膜/瞳孔分层合成（多贴图 + 正确的正面 UV 映射），后续再做。
